<!DOCTYPE html>
<html lang="en">
        <head>
<!--            --><?php //include "property-head.php";?>
            <?php include "htmlheader.php";?>

            <title>ORG Property Management | Properties</title>
            </head>
<!--    <body id="property">-->
    <body>
        <header class="header" id="properties">

<!--            --><?php //include "property-nav.php";?>

            <?php include "main_nav.php";?>
            <div class="service__container">
                <div class="service-content">
                    <h1 class="text-service">Properties</h1>
                </div>
<!--                <hr class="hr-line">-->
<!--                <span> Lorem ipsum, dolor sit amet consectetur adipisicing elit. Blanditiis, molestiae ab. Expedita possimus voluptates, laudantium provident voluptas</span>-->
               
             <!-- <span>ORG Portfolio Management LLC offers the services below to institutional
                  investors on a full retainer or unbundled basis. ORG can serve in a fiduciary capacity
                  pursuant to ERISA requirements.</span> -->
            </div>
        </header>

        <section class="section-homes" id="stickyy">

            <?php include "property-cards.php";?>

            <?php include "property-card-1.php";?>


            <?php include "property-card-2.php";?>

            <?php include "property-card-3.php";?>

            <?php include "property-card-4.php";?>


            <?php include "property-card-5.php";?>

            <?php include "property-card-6.php";?>

            <?php include "property-card-7.php";?>

<!--            --><?php //include "property-card-8.php";?>

            <?php include "property-card-9.php";?>

            <?php include "property-card-10.php";?>

            <?php include "property-card-11.php";?>

            <?php include "property-card-12.php";?>


        </section>

        <?php include "portfoliofooter.php";?>

       <!-- <?php /*include "property-footer.php";*/?>
        --><?php /*include "general-footer.php";*/?>

    </body>
</html>
